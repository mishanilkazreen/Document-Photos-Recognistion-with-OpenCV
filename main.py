...
class OCRProcessor:
    ...
    def predict(self, img_data: ndarray):
        return self.ocr.predict(img_data)


class ImageResizer:
    # Function: Resizing image data with an aspect ratio given width
    def resize_to_width(self, img_data: ndarray, target_width: int) -> tuple[float, ndarray]:
...
class CoordinateScaler:
    ...
    def scale_coordinates(self, coordinates_data: ndarray, scale_factor: float) -> ndarray:
...
class ImageLoader:
    ...
    def load_img_into_memory(self, directory: str) -> list:
        img_data_arr = []
        ...
        for file_name in image_files:
            data = self.convert_img_to_data(file_name)
...
class CannyProcessor:
    ...
    def apply_canny_on_img(self, img_data: ndarray) -> ndarray:
        canny_params = dict(
            threshold1=80,
            threshold2=120
        )
...
class LineIntersector:
    ...
    def find_intersection(self, line_1: ndarray, line_2: ndarray) -> ndarray:
...
class DocumentCornerExtractor:
    ...
    def extract_document_corners(self, img_data: ndarray) -> ndarray:
        line_intersections = []
        ...
        canny_img = self.canny_processor.apply_canny_on_img(img_data)

        # [https://learnopencv.com/hough-transform-with-opencv-c-python/](https://learnopencv.com/hough-transform-with-opencv-c-python/)
        ...
        for i in range(len(lines)):
            for v in range(i + 1, len(lines)):
                cords = self.line_intersector.find_intersection(lines[i], lines[v])
...
class PerspectiveWarper:
    ...
    def apply_perspective_warp(self, img_data: ndarray, corners: ndarray) -> ndarray:
        a4_width = 1000
...
class DocumentProcessor:
    def __init__(self, lang):
        ...
        self.corner_extractor = DocumentCornerExtractor()
        self.warper = PerspectiveWarper()
        self.ocr_processor = OCRProcessor(lang)

    def process(self, file_directory: str):
        img_files = self.image_loader.load_img_into_memory(file_directory)

        for img_file in img_files:
            aspect_ratio, img_file_downsized = self.image_resizer.resize_to_width(img_file, 800)  # Improves performance with pre-processing when scaling down

            corner_data_downsized = self.corner_extractor.extract_document_corners(img_file_downsized)
            corner_data_normal = self.coordinate_scaler.scale_coordinates(corner_data_downsized, aspect_ratio)  # Return coordinates for the original image

            warped_img = self.warper.apply_perspective_warp(img_file, corner_data_normal)

            result = self.ocr_processor.predict(warped_img)
...
if __name__ == '__main__':
    ...
    processor = DocumentProcessor(settings.LANG)
    processor.process(file_to_load)
